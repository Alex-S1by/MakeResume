...
export default function Home() {
  ...
  const makeresume = () => { window.location.href = "/builder"; };
  const closePopup = () => {
  ...
  const improveResume = async () => {
    ...
    try {
      ...
      if (!text || text.length < 10) {
        setLoading(false);
        setErrorMsg("Extracted resume text is too short or empty. Please try a different file.");
        return;
      }
      ...
      if (data) localStorage.setItem("resume_draft", JSON.stringify(data.improved));
      ...
      setLoading(false);
      setErrorMsg(
        err.message || "AI is busy right now. Please try again in a moment."
      );
    }
  };
      ...
      <div className="min-h-screen bg-white text-gray-900" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-2">
              <button className="hidden sm:inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold text-gray-700 rounded-lg hover:bg-gray-100 transition-colors">
                <i className="fa-regular fa-user text-xs"/> Login
              </button>
          ...
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              <span className="inline-flex items-center gap-2 bg-sky-50 text-sky-600 border border-sky-100 text-xs font-bold px-3 py-1.5 rounded-full mb-5 tracking-wide uppercase">
                <i className="fa-solid fa-bolt"/> AI-Powered Resume Builder
              </span>
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-7">
                <button onClick={makeresume} className="inline-flex items-center justify-center gap-2 bg-sky-500 hover:bg-sky-600 text-white font-bold text-sm px-6 py-3.5 rounded-xl transition-all shadow hover:shadow-sky-200 hover:shadow-lg">
                  <i className="fa-solid fa-wand-magic-sparkles"/> Create My Resume
                </button>
                <button onClick={openPopup} className="inline-flex items-center justify-center gap-2 border border-gray-200 hover:border-gray-300 bg-white text-gray-700 font-semibold text-sm px-6 py-3.5 rounded-xl transition-colors hover:bg-gray-50">
                  <i className="fa-solid fa-upload text-sky-500"/> Upload & Analyze
                </button>
              </div>
              <div className="flex flex-wrap gap-4 items-center">
                ...
                <div className="flex items-center gap-1.5 text-xs text-gray-500 font-medium">
                  <i className="fa-solid fa-shield-check text-sky-500"/> ATS-Friendly
                </div>
                <span className="w-px h-4 bg-gray-200"/>
                <div className="flex items-center gap-1.5 text-xs text-gray-500 font-medium">
                  <i className="fa-solid fa-users text-sky-500"/> 10k+ users
                </div>
              </div>
            </div>
            ...
            <div className="relative flex justify-center md:justify-end mt-6 md:mt-0">
              ...
              <div className="relative z-10">
                <div className="absolute -top-4 -left-3 sm:-left-6 z-20 bg-white border border-gray-100 shadow-xl rounded-2xl px-3 py-2.5 flex items-center gap-2.5">
                  <div className="w-9 h-9 rounded-xl bg-sky-500 flex items-center justify-center text-white font-extrabold text-sm shrink-0">94</div>
                  <div>
                ...
                <div className="rounded-2xl overflow-hidden border border-gray-100 shadow-2xl shadow-gray-200/80">
                  <Image src="/heroimg.webp" alt="Resume Preview" width={380} height={480} className="block w-full max-w-[260px] sm:max-w-xs md:max-w-sm"/>
                </div>
        ...
        <div className="border-y border-gray-100 bg-gray-50/70 py-6">
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <p className="text-center text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-5">Trusted by job seekers hired at</p>
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 py-14 md:py-20">
          <div className="text-center mb-10">
            <span className="inline-flex items-center gap-2 bg-sky-50 text-sky-600 border border-sky-100 text-xs font-bold px-3 py-1.5 rounded-full mb-4 uppercase tracking-wide">
              <i className="fa-solid fa-sparkles"/> Features
            </span>
            <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-gray-900">Everything you need to land the job</h2>
            <p className="text-gray-500 mt-3 max-w-xl mx-auto text-sm sm:text-base">From building to optimizing — we handle the hard parts so you can focus on the interview.</p>
          </div>
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-5">
            {[
              ...
              <div key={f.title} className="group bg-white border border-gray-100 rounded-2xl p-6 hover:shadow-lg hover:shadow-gray-100 hover:-translate-y-1 transition-all duration-200 cursor-pointer">
                <div className={`w-11 h-11 rounded-xl ${f.bg} flex items-center justify-center text-base mb-4`}><i className={f.icon}/></div>
                ...
                <span className="inline-flex items-center gap-1.5 text-xs font-bold text-sky-500 group-hover:gap-3 transition-all">Learn more <i className="fa-solid fa-arrow-right text-[10px]"/></span>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 pb-14 md:pb-20">
          <div className="text-center mb-10">
            ...
            <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-gray-900">Ready in 3 simple steps</h2>
          </div>
          <div className="grid sm:grid-cols-3 gap-5">
            {[
              ...
              <div key={s.step} className="flex flex-col items-center text-center bg-white rounded-2xl border border-gray-100 p-8 shadow-sm hover:shadow-md transition-shadow">
                <div className="w-14 h-14 rounded-2xl bg-sky-500 text-white flex items-center justify-center text-xl mb-4 shadow-md shadow-sky-200/60"><i className={s.icon}/></div>
        ...
        <section className="max-w-6xl mx-auto px-4 sm:px-6 pb-14 md:pb-20">
          <div className="bg-sky-500 rounded-3xl p-8 sm:p-12 flex flex-col sm:flex-row items-center justify-between gap-6 relative overflow-hidden">
            ...
            <div className="relative z-10 text-center sm:text-left">
              <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-white mb-2">Your dream job is one resume away.</h2>
              <p className="text-sky-100 text-sm sm:text-base">Start for free — no credit card required.</p>
            </div>
        ...
        {/* ── UPLOAD POPUP ── */}
        <div id="popup-box" className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 hidden p-4">
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl w-full max-w-md p-4 sm:p-6 relative animate-scaleIn max-h-[90vh] overflow-y-auto">

            {/* ── DEFAULT STATE ── */}
            {!loading && !errorMsg && (
              <>
                <button onClick={closePopup} className="absolute top-3 right-3 text-gray-400 hover:text-gray-600 text-lg sm:text-xl">×</button>
                <h2 className="text-lg sm:text-2xl font-semibold mb-2 text-gray-800 text-center sm:text-left">Upload Resume</h2>
                <p className="text-gray-500 mb-5 text-xs sm:text-sm text-center sm:text-left">DOC, DOCX, or PDF (max 10MB)</p>

                <label onClick={()=>fileRef.current?.click()} className="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-lg sm:rounded-xl p-4 sm:p-6 cursor-pointer hover:border-sky-500 transition mb-4">
                  ...
                  {fileName && (
                    <div className="flex items-center justify-between mt-2 w-full">
                      <span className="text-sky-600 text-xs sm:text-sm font-medium break-all">{fileName}</span>
                      <button type="button" onClick={e=>{e.stopPropagation();viewFile();}} className="text-sky-600 hover:text-sky-800 text-sm flex items-center gap-1">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <button onClick={closebtn} className="w-full sm:flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium py-2.5 rounded-lg transition">Cancel</button>
                  <button onClick={improveResume} className="w-full sm:flex-1 bg-sky-500 hover:bg-sky-600 text-white font-medium py-2.5 rounded-lg shadow-md hover:shadow-lg transition">Improve Resume</button>
                </div>
              </>
            )}
            ...
            {loading && !errorMsg && (
              <div className="flex flex-col items-center justify-center py-10 gap-5 text-center">
                ...
                <h3 className="text-lg font-semibold text-gray-800">Improving your resume</h3>
                <p className="text-gray-500 text-sm transition-all duration-300">{steps[loadingStep]}</p>
                <div className="flex gap-2 mt-2">
                  {steps.map((_,i)=>(
                    <div key={i} className={`w-2 h-2 rounded-full ${i<=loadingStep?"bg-sky-500":"bg-gray-300"}`}/>
                  ))}
                </div>
              </div>
            )}
            ...
            {errorMsg && (
              <div className="flex flex-col items-center justify-center py-10 gap-5 text-center">
                <div className="w-12 h-12 rounded-full bg-red-100 flex items-center justify-center">
                  <i className="fa-solid fa-exclamation text-red-500"/>
                </div>
                ...
                <button onClick={()=>window.location.reload()} className="mt-3 bg-sky-500 hover:bg-sky-600 text-white px-4 py-2 rounded-lg text-sm">Try Again</button>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
              <a href="#" className="flex items-center font-extrabold text-gray-900">
                <span className="w-7 h-7 rounded-lg bg-sky-500 flex items-center justify-center text-white text-xs"><i className="fa-solid fa-file-lines"/></span>
              ...
              <div className="flex items-center gap-2.5">
                {[{icon:"fa-brands fa-x-twitter",label:"Twitter"},{icon:"fa-brands fa-linkedin-in",label:"LinkedIn"},{icon:"fa-brands fa-github",label:"GitHub"}].map(s=>(
                  <a key={s.label} href="#" aria-label={s.label} className="w-9 h-9 rounded-lg border border-gray-200 flex items-center justify-center text-gray-400 hover:border-sky-500 hover:text-sky-500 transition-colors">
